...
현재 이더리움의 샤딩을 "Danksharding"이라고 하는데, 매 슬롯마다 64개의 샤드를 나누어(한 슬롯의 최대 커미티 수는 64개) 데이터를 처리하는 대신 하나의 블록 빌더가 L2의 롤업 데이터를 전부 처리하게 한다. 하지만 이렇게 되면 블록을 만드는 리소스가 더 많이 요구 되므로 PBS를 통해 다수의 "일반" 검증자와 소수의 "전문" 블록 빌더의 역할을 분담하는 것이 필수적이다.  
