...
4. `chainId`와 전자서명  
이더리움 트랜잭션의 전자서명(r,s,v)에서 마지막 한 바이트(v 값)는 처음 32바이트(r 값)가 짝수일 때 27(0x1b), 홀수일 때 28(0x1c)이었는데 EIP-155 이후 트랜잭션에 `chainId`가 포함되면서 전자서명의 마지막 한 바이트(v)를 chainId를 사용하여 계산된 값이 들어가도록 변경되었다.  
