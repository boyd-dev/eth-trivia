...
EIP-1559는 2021년 8월 런던 하드포크에 적용된 "fee market" 즉 가스비 체계의 변경이다. 이 개선안은 그동안 가스비가 수요에 따라 변동폭이 심하고, 때로는 급격하게 오르는 것을 완화하려는 목적이 있다. 근본적으로는 수요와 공급에 의해 가스비가 오르는 것을 해결할 수는 없지만 사용자가 막연하게 가스비를 추산하는 방식에서 벗어나 어느정도 예측가능한 가스비, 즉 가스비를 이 정도 내면 즉시 처리될 수 있다는 것을 알려주는 "표준 요금표"를 제공하는 것이 목적이다.  
1559에 새로 도입된 개념이 "기본 수수료(BASE FEE)"인데, 말 그대로 기본 수수료+ɑ 정도만 부담하면 트랜잭션이 다음 블록에서 항상 처리되는 것을 보장한다. ɑ를 "priority fee"라고 부른다. 이렇게 하면 이더리움 자체에서 기본 수수료가 계산되므로 사용자가 내는 전체 수수료(기본 수수료+ɑ)의 수준을 미리 파악할 수 있다는 장점이 있다. 기본 수수료의 존재는 현재 네트워크의 트랜잭션 수요를 나타내는 것으로 볼 수 있고 priority fee는 블록 생성자가 트랜잭션을 더 빨리 처리하도록 유인하는 "팁"에 해당한다. 이것은 기존과 큰 차이가 없다. 그래서 1559는 가스비가 저렴해지는 것과는 직접적인 관련이 없다.  
...
기본 수수료는 최대 인상폭이 12.5%로 상한이 정해져 있기 때문에 급격하게 오르지 않도록 되어 있다. 사용량이 1500만에서 상한인 3000으로 2배가 늘어나면 기본 수수료는 12.5%가 인상되는 셈이다. 물론 계속 상한까지 채워지면 매 블록마다 12.5%씩 증가하고 사용자들이 정하는 ɑ도 있으므로 여전히 높은 가스비가 지속될 수는 있다.  

   1559 이전에는 모든 수수료가 블록 생성자에게 지급되었지만 도입 후에는 기본 수수료가 소각된다. 따라서 블록 생성자에게 돌아가는 트랜잭션 처리 수수료는 ɑ에 해당하는 priority fee 뿐이다(덕분에 다른 수익원인 MEV에 대한 중요도가 높아졌다). 또 수수료를 반드시 이더로 내야 하기 때문에 소위 말하는 "economic abstraction"을 방지하는 효과도 있다. 1559의 소각과 지분증명 전환으로 인해 신규 이더의 발행량이 실질적으로 마이너스가 되기 때문에 이더리움을 ["ultra sound money"](https://ultrasound.money)라고 말하기도 한다.  
