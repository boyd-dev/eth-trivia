...
이더리움에서 "계정 추상화(Account Abstraction, AA)"는 사용자 UI/UX 관점에서 오랫동안 논의된 주제라고 할 수 있다. 이더리움의 "계정"은 타원곡선암호 기반으로 생성하고(오프체인에서 만들기 때문에 Externally Owned Account) ECDSA를 사용하여 트랜잭션에 서명한다. 여러 EIP들이 있었으며 대략 두 가지의 제안으로 압축되었고 그것이 바로 [EIP-3074](https://eips.ethereum.org/EIPS/eip-3074)와 [ERC-4337](https://eips.ethereum.org/EIPS/eip-4337)이다.  
   ...
   현재는 ERC-4337이 사실상 표준으로 채택된 상태지만 최근 들어 다시 EIP-3074를 적용하자는 의견들이 많아지고 결국 2024년 하반기 예정인 "Pectra" 하드포크에 포함시키기로 잠정 [합의](https://twitter.com/TimBeiko/status/1778497101988835715)하기에 이른다. 이름에서 알 수 있는 것처럼 4337은 ERC로 하드포크 없이 [적용](https://www.bundlebear.com)되는 것이 가능하지만 3074는 EIP에 속하기 때문에 프로토콜의 변경이 필요하다.  

   ERC-4337이 이미 실행 중인 상태에서 EIP-3074를 다시 적용한다는 것은 다소 이해할 수 없는 측면이 있다. AA라는 것은 원래, 사실 "추상화"로 번역하기 보다는 EOA 계정을 프로토콜에서 "추출"한다는 개념에 가까운데, 그렇게 보면 EOA를 <b>스마트 컨트랙트</b> 계정으로 전환한다는 의미가 있는 것이다. 그런데 EIP-3074는 EOA를 그대로 유지하면서 EOA의 기능을 확장하는 것에 가깝다. 개인키가 유출되면 모든 자산이 위험해지는 것은 여전하다. 반면 ERC-4337은 스마트 컨트랙트 지갑을 전제로 하며 EOA에 있는 이더와 토큰을 스마트 컨트랙트로 옮겨야 한다. 당연히 스마트 컨트랙트 기반으로 동작하므로 활용도가 높다. 그래서 EIP-3074가 적용되더라도 궁극적으로는 ERC-4337이 제공하는 것과 같은 스마트 컨트랙트들이 필요하기 때문에 EIP-3074가 ERC-4337을 대체할 수는 없다.  
   
   한편으로는 EIP-3074는 EOA의 확장이기 때문에 모든 이더리움 계정들을 프로토콜 변경으로, 자동적으로 계정 추상화할 수 있다는 것이 장점이다. ERC-4337은 애플리케이션 레벨이기 때문에 사용자의 선택에 달려 있다. 즉 여전히 EOA를 사용할 수 있고 실제로 현재 AA 사용률은 매우 낮다. EIP-3074는 바로 이러한 상황을 완전히 바꿀 수 있다(하드포크하기 때문에 당연한 일인지도). EOA는 그 자체로(out of box) 스마트 컨트랙트 계정이 되도록 만들자는 것이 EIP-3074의 주장이다. EIP-3074 기반에서 ERC-4337이 상호 보완적으로 이루어지는 것이 바람직한 방향일 수 있겠으나 현재는 두 가지 방안이 대결 양상을 보이는 구도가 된 측면이 없지 않다. 아마 EIP-3074이 Pectra 하드포크에 포함될 것이라는 소식이 전해지자 ERC-4337을 기반으로 하는 개발팀들의 반발이 있었던 것으로 보인다.
   
   결국 비탈릭 부테린을 중심으로 [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702)를 다시 제안하면서 AA에 대한 잡음이 가라앉고 있는 것 같다.
