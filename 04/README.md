...
이더리움에서 "계정 추상화(Account Abstraction, AA)"는 사용자 UI/UX 관점에서 오랫동안 논의된 주제라고 할 수 있다. 이더리움의 "계정"은 타원곡선암호 기반으로 생성하고(오프체인에서 만들기 때문에 Externally Owned Account) ECDSA를 사용하여 트랜잭션에 서명한다. 여러 EIP들이 있었으며 대략 두 가지의 제안으로 압축되었고 그것이 바로 [EIP-3074](https://eips.ethereum.org/EIPS/eip-3074)와 [ERC-4337](https://eips.ethereum.org/EIPS/eip-4337)이다.  
   ...
   한편으로는 EIP-3074는 EOA의 확장이기 때문에 모든 이더리움 계정들을 프로토콜 변경으로, 자동적으로 계정 추상화할 수 있다는 것이 장점이다. ERC-4337은 애플리케이션 레벨이기 때문에 사용자의 선택에 달려 있다. 즉 여전히 EOA를 사용할 수 있고 실제로 현재 AA 사용률은 매우 낮다. EIP-3074는 바로 이러한 상황을 완전히 바꿀 수 있다(하드포크하기 때문에 당연한 일인지도). EOA는 그 자체로(out of box) 스마트 컨트랙트 계정이 되도록 만들자는 것이 EIP-3074의 주장이다.  
   
   EIP-3074 기반에서 ERC-4337이 상호 보완적으로 이루어지는 것이 바람직한 방향일 수 있겠으나 현재는 두 가지 방안이 대결 양상을 보이는 구도가 된 측면이 없지 않다. 아마 EIP-3074이 Pectra 하드포크에 포함될 것이라는 소식이 전해지자 ERC-4337을 기반으로 하는 개발팀들의 반발이 있었던 것으로 보인다.
